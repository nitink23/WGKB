...
def display_circos_plot(data: dict, genomic_ranges: list, chrom_dict: dict, gene_colors: dict, full_data, all_genes, \
    ...
    if full_data is not None:
        ...
        bar_color = st.color_picker(f"Pick a color for bar plot on bar track", '#ff0000', key=f"color_picker_bar_track")
    ...
    for index, sector_obj in enumerate(circos.sectors):
        ...
        # Add colorbar on the right side to represent avg_exp_lvl range
        circos.colorbar(
            bounds=(0.9, 0.9, 0.02, 0.4),  # Adjust bounds to place it on the right side
            vmin=0,
            vmax=full_data[gene_exp_avg_exp].max(),
            cmap="coolwarm",  # Choose a suitable color map
            orientation="vertical",  # Set to vertical for a right-side scale
